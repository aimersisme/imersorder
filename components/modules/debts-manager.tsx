...
export function DebtsManager({ businessId }: { businessId: string }) {
  ...
  return <>
    <ModuleHeader title="Hutang & Piutang Pribadi" subtitle="Catat pinjaman pribadi tanpa Order atau Invoice" actionLabel="Tambah" onAction={()=>openCreate()} />
    {notice?<Notice kind={notice.kind}>{notice.text}</Notice>:null}
    <section className="debtKpis"><article className="kpiGreen"><ArrowDownLeft size={18}/><span>Piutang Pribadi</span><strong>{formatIDR(receivable)}</strong><small>Orang lain berutang ke usaha</small></article><article className="kpiDanger"><ArrowUpRight size={18}/><span>Hutang Pribadi</span><strong>{formatIDR(payable)}</strong><small>Kewajiban usaha</small></article><article className="kpiAmber"><CalendarClock size={18}/><span>Jatuh Tempo</span><strong>{formatIDR(overdue)}</strong><small>Piutang/hutang melewati tempo</small></article></section>
    <div className="segmented"><button className={type==="all"?"active":""} onClick={()=>setType("all")}>Semua Pribadi</button><button className={type==="receivable"?"active":""} onClick={()=>setType("receivable")}>Piutang Pribadi</button><button className={type==="payable"?"active":""} onClick={()=>setType("payable")}>Hutang Pribadi</button><button onClick={()=>openCreate("receivable")}><Plus size={14}/> Piutang</button><button onClick={()=>openCreate("payable")}><Plus size={14}/> Hutang</button></div>
    ...
    {selected?<div className="modalBackdrop"><section className="modalCard formPanel"><div className="modalHead"><div><h2>{selected.person_name}</h2><p>{selected.type==="receivable"?"Piutang Pribadi":"Hutang Pribadi"} · saldo {formatIDR(selected.balance_amount)}</p></div><button className="modalClose" onClick={()=>setSelected(null)}><X size={18}/></button></div><div className="detailStats"><div className="detailStat"><span>Nominal awal</span><strong>{formatIDR(selected.original_amount)}</strong></div><div className="detailStat"><span>Sisa saldo</span><strong>{formatIDR(selected.balance_amount)}</strong></div><div className="detailStat"><span>Tanggal</span><strong>{formatDate(selected.transaction_date)}</strong></div><div className="detailStat"><span>Jatuh tempo</span><strong>{formatDate(selected.due_date)}</strong></div></div><div className="sectionTitleRow" style={{marginTop:16}}><div><h2 style={{fontSize:14}}>{selected.type==="receivable"?"Catat Pembayaran":"Catat Pembayaran Hutang"}</h2><p>{selected.type==="receivable"?"Pembayaran yang diterima akan mengurangi saldo piutang secara otomatis.":"Pembayaran yang dilakukan akan mengurangi saldo hutang secara otomatis."}</p></div></div>{Number(selected.balance_amount)>0?<form className="fieldGrid" onSubmit={addPayment}><label className="formField"><span>Nominal</span><input type="number" min="1" max={Number(selected.balance_amount)} value={paymentAmount} onChange={e=>setPaymentAmount(e.target.value)} placeholder="2000000" required/></label><label className="formField"><span>Tanggal</span><input type="date" value={paymentDate} onChange={e=>setPaymentDate(e.target.value)} required/></label><label className="formField full"><span>Catatan</span><input value={paymentNote} onChange={e=>setPaymentNote(e.target.value)} placeholder="Cicilan ke-1"/></label><div className="formActions" style={{gridColumn:"1/-1"}}><button className="primaryAction" disabled={saving} type="submit"><CreditCard size={16}/> {saving?"Menyimpan...":selected.type==="receivable"?"Catat Pembayaran":"Catat Pembayaran Hutang"}</button></div></form>:<Notice>Data sudah lunas.</Notice>}<div className="sectionTitleRow" style={{marginTop:16}}><div><h2 style={{fontSize:14}}>Riwayat Pembayaran</h2></div></div><div className="tableLike">{payments.length===0?<div className="tableRow"><span>Belum ada pembayaran.</span></div>:payments.map(p=><div className="tableRow" key={p.id}><div><strong>{formatDate(p.payment_date)}</strong><small>{p.note||"Pembayaran"}</small></div><strong>{formatIDR(p.amount)}</strong></div>)}</div></section></div>:null}
  </>;
}
