...
export function TeamManager({ businessId, role }: { businessId: string; role: string }) {
  ...
  const [linkLabel, setLinkLabel] = useState("Link Undangan");
  ...
  async function copyLink() {
    ...
    try {
      ...
      setCopiedLink(true);
      setMsg({ kind: "success", text: `${linkLabel} berhasil disalin.` });
      window.setTimeout(() => setCopiedLink(false), 1800);
    } catch {
      setMsg({ kind: "error", text: "Browser tidak mengizinkan penyalinan otomatis. Silakan salin link secara manual." });
    }
  }
  ...
  async function resetPassword(m: Member) {
    ...
    setResettingUserId(m.user_id);
    setMsg(null);
    try {
      const response = await fetch("/api/team/password-reset", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ businessId, userId: m.user_id }),
      });
      ...
      if (data.resetUrl) {
        ...
        setLinkLabel("Link Reset Password");
        setCopiedLink(false);
      }
      if (!response.ok) throw new Error(data.error || "Gagal membuat link reset password.");
      setMsg({
        kind: "success",
        text: data.message || "Link reset password berhasil dibuat.",
      });
    } catch (error) {
      setMsg({ kind: "error", text: error instanceof Error ? error.message : "Gagal membuat link reset password." });
    } finally {
    ...
    <>
      ...
      {isOwner ? (
        <form className="formPanel" onSubmit={invite}>
          ...
          {link ? (
            <div className="formField" style={{ marginTop: 12 }}>
              {linkLabel}
              <div className="copyLinkRow">
                <input readOnly value={link} onFocus={(e) => e.currentTarget.select()} />
                <button type="button" className={`miniButton ${copiedLink ? "primary" : ""}`} onClick={() => void copyLink()}>
                  {copiedLink ? <Check size={15} /> : <Copy size={15} />}
                  {copiedLink ? "Tersalin" : "Salin Link"}
                </button>
              </div>
              <span className="formHint">{linkLabel === "Link Undangan" ? "Berlaku 7 hari. Anggota membuat password sendiri dari link undangan; password tidak pernah ditampilkan kepada Owner." : "Gunakan link ini untuk membantu anggota membuat password baru."}</span>
            </div>
          ) : null}
        </form>
      ) : null}
            ...
            <article className="crudCard" key={m.id}>
              <div className="crudCardTop">
                <div className="crudCardTitle">
                  <strong>{p.full_name || "Pengguna iMersOrder"}</strong>
              ...
              {isOwner && !isMemberOwner ? (
                <div className="crudCardActions">
                  ...
                  >
                    {resettingUserId === m.user_id ? "Membuat..." : "Reset Password"}
                  </button>
                  <button
                    type="button"
