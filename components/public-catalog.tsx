...
export function PublicCatalog({data}:{data:Data}){
  ...
  return <main className="publicCatalog">
    ...
    {promoOpen ? <div className="catalogOverlay promoOverlay" onClick={()=>setPromoOpen(false)}><div className="promoPopup" role="dialog" aria-modal="true" aria-label={String(popup.title ?? "Promo & Pengumuman")} onClick={e=>e.stopPropagation()}><button className="modalClose" onClick={()=>setPromoOpen(false)}>×</button><div className="promoPopupIcon">✦</div><span className="promoPopupEyebrow">INFO & PROMO</span><h2>{String(popup.title ?? "Promo & Pengumuman")}</h2><p>{String(popup.text ?? "")}</p><div className="promoPopupActions"><button className="catalogSecondary" onClick={()=>setPromoOpen(false)}>Nanti</button><a className="catalogPrimary promoPopupButton" href={popupUrl} target="_blank" rel="noopener noreferrer">{String(popup.button ?? "Lihat Promo")}</a></div></div></div> : null}
  </main>;
}
