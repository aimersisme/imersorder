...
begin
  ...
  if coalesce(p_seed_demo, false) then
    ...
    perform public.seed_catering_demo_transactions(v_business_id, v_user);
  end if;
...
-- ============================================================
-- iMersOrder v0.2.1 - Performance + UI data patch
...
commit;
-- ============================================================
-- iMersOrder v0.2.3 — Automatic Catalog SKU
...
-- ============================================================
-- iMersOrder v0.2.8 — Dashboard Personalization Patch
...
-- ============================================================
-- iMersOrder v0.2.9 - Single auth / invitation preview
-- ============================================================
...
-- ============================================================
-- PESANLUNAS v0.2.11-r7 — WHATSAPP INTEGRATION CREDENTIALS + SERVER RPC
-- ============================================================
...
-- ============================================================
-- v0.2.11-r8 AUTH / TEAM PASSWORD FLOW
-- Invitation members create their own password from the invitation link.
-- Owner never receives or stores member passwords. Active members can be
-- helped by Owner through the server-side password recovery flow.
-- SUPABASE_SERVICE_ROLE_KEY is required by the Next.js team reset route.
-- ============================================================
-- iMersOrder v0.2.11-r9
...
-- ============================================================
-- iMersOrder v0.2.11-r10: team invitation preview fix
-- ============================================================
-- iMersOrder v0.2.11-r10 HOTFIX
-- Fix team invitation preview still calling digest(), which caused:
-- function digest(text, unknown) does not exist
-- This patch only replaces the invitation preview validator with md5(),
-- matching the r9 invitation create/accept functions.
...
create index if not exists catalog_items_business_category_idx
  on public.catalog_items(business_id, category)
  where deleted_at is null;
...
grant execute on function public.submit_public_catalog_order(jsonb) to anon, authenticated;

insert into public.business_settings(business_id,key,value)
select id,'catalog_enabled','{"enabled":false}'::jsonb from public.businesses
on conflict (business_id,key) do nothing;
insert into public.business_settings(business_id,key,value)
select id,'catalog_description','{"text":"Bagikan katalog dan terima pesanan online dengan lebih rapi."}'::jsonb from public.businesses
on conflict (business_id,key) do nothing;
insert into public.business_settings(business_id,key,value)
select id,'catalog_show_prices','{"enabled":true}'::jsonb from public.businesses
on conflict (business_id,key) do nothing;
insert into public.business_settings(business_id,key,value)
...
-- ============================================================
-- iMersOrder r18: branding everywhere (login + public invoice)
-- ============================================================
