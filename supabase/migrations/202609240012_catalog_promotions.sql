-- iMersOrder r25: optional public catalog promo popup + marquee settings.
-- No new tables are required; business_settings stores the JSON configuration.

insert into public.business_settings(business_id,key,value)
select id,'promo_popup','{"enabled":false,"title":"Promo & Pengumuman","text":"Ada promo terbaru untuk pelanggan Anda.","button":"Lihat Promo","url":"","delay_seconds":3}'::jsonb
from public.businesses
on conflict (business_id,key) do nothing;

insert into public.business_settings(business_id,key,value)
select id,'promo_marquee','{"enabled":false,"text":"🔥 Promo terbaru tersedia — klik untuk melihat detail.","url":""}'::jsonb
from public.businesses
on conflict (business_id,key) do nothing;
